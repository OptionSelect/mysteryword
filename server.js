...
app.post('/', (req, res) => {
  let playing = 'Y'
  req.checkBody('guess', 'You need to put guess a LETTER!.').notEmpty().isLength(0, 1).isAlpha()
  ...
  if (errors) {
    ...
    errors = { msg: 'You already guessed that! Pick another letter!' }
    res.render('home', {
      underscores: underscores,
      guesses: guesses,
      count: count,
    ...
    if (count === 0) {
      ...
      let winCondish = 'W'
      res.render('home', { underscores: underscores, guesses: guesses, count: count, winCondish: winCondish })
    }
  }
})
